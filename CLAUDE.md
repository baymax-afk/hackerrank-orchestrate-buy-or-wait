...
Current state: the solution is implemented under `code/` (see `code/README.md` for the module map) with pytest tests under `tests/`, research notes under `research/`, and the calibrated deterministic core scoring 20/25 statuses on the samples (`python code/main.py --sample-check --no-llm`).
