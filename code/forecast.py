...
class Timeline:
    ...
    points: tuple[tuple[date, Decimal], ...]  # end-of-day balances on flow dates, ascending
    ...
    def min_from(self, d: date) -> Decimal:
        """Minimum end-of-day balance over [d, horizon end] — the quantity a payment on d must protect."""
        m = self.balance_on(d)
        for t, b in self.points:
            if t > d and b < m:
                m = b
